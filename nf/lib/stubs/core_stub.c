...
#define KLEE_TRACE_MBUF_CONTENT(u_ptr, dir)                                                                             \
  klee_trace_extra_ptr(u_ptr, sizeof(struct stub_mbuf_content), "user_buf_addr", "", "MBUF", dir);                      \
  ...
  {                                                                                                                     \
    klee_trace_extra_ptr_nested_nested_field(u_ptr,                                                                     \
...
bool stub_core_mbuf_create(uint16_t device, struct rte_mempool *pool, struct rte_mbuf **mbufp)
{
  ...
  *mbufp = rte_mbuf_raw_alloc(pool);
  ...
  if (RTE_ETH_IS_IPV4_HDR((*mbufp)->packet_type) || ((*mbufp)->packet_type == 0 && buf_content->ether.ether_type == rte_cpu_to_be_16(ETHER_TYPE_IPv4)))
  {
