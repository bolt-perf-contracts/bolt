...
int dchain_allocate(int index_range, struct DoubleChain** chain_out) {
    ...
    *chain_out = malloc(sizeof(struct DoubleChain));
    ...
    klee_trace_extra_ptr(&is_dchain_allocated, sizeof(is_dchain_allocated),
                         "is_dchain_allocated", "type", TD_BOTH);
    klee_trace_extra_ptr(&index_range, sizeof(index_range),
                         "dchain_index_range", "type", TD_BOTH);
...
int dchain_allocate_new_index(struct DoubleChain* chain, int *index_out,
    ...
    ALLOW(chain);
    klee_trace_extra_ptr(&chain->dchain_out_of_space, sizeof(chain->dchain_out_of_space), "dchain_out_of_space", "type", TD_BOTH);
