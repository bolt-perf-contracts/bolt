...
#include "map-stub-control2.h"

int has_this_key_a;
int map2_allocate(map_keys_equality* keq, map_key_hash* khash,
                 int capacity,
                 struct Map** map_out) {
  klee_trace_ret();
  ...
  int allocation_succeeded = klee_int("map2_allocation_succeeded");
  klee_trace_extra_ptr(&allocation_succeeded, sizeof(allocation_succeeded),
                       "map2_allocation_succeeded", "type", TD_BOTH);
  klee_trace_extra_ptr(&capacity, sizeof(capacity),
                       "map2_capacity", "type", TD_BOTH);
  if (allocation_succeeded) {
    *map_out = malloc(sizeof(struct Map));
    ...
    (*map_out)->ent_cond = 0;
    (*map_out)->occupancy = klee_range(0, capacity, "map2_occupancy");
    (*map_out)->backup_occupancy = klee_range(0, capacity, "map2_backup_occupancy");
    (*map_out)->Num_bucket_traversals = klee_int("Num_bucket_traversals2");
    (*map_out)->Num_hash_collisions = klee_int("Num_hash_collisions2");
    return 1;
  }
  return 0;
}

void map2_reset(struct Map* map) {
  //Do not trace. This function is an internal knob of the model.
  for (int n = 0; n < NUM_ELEMS; ++n) {
    map->keyp[n] = NULL;
    map->key_deleted[n] = 0;
    map->allocated_index[n] = klee_int("map2_allocated_index");
  }
...
void map2_set_layout(struct Map* map,
...
void map2_set_entry_condition(struct Map* map, map_entry_condition* cond) {
  map->ent_cond = cond;
}
...
void map2_increase_occupancy(struct Map* map, int change) {
  map->occupancy = map->occupancy + change;
}

void map2_decrease_occupancy(struct Map* map, int change) {
  map->occupancy = map->occupancy - change;
}
...
int map2_get_1(struct Map* map, void* key, int* value_out) {
  ...
  klee_make_symbolic(&(has_this_key_a), sizeof(has_this_key_a), "map2_has_this_key_a");
  klee_trace_extra_ptr(&(has_this_key_a), sizeof(has_this_key_a), "map2_has_this_key_a", "type", TD_BOTH);
