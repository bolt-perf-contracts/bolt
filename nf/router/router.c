...
int nf_core_process(struct rte_mbuf* mbuf, time_t now)
{
	...
	if (gettimeofday(&tv, NULL) == 0) {
		current_milliseconds_utc = ((tv.tv_sec % 86400) * 1000 + tv.tv_usec / 1000);
	} else {
		return mbuf->port;
	}
	...
	if (ip_header != NULL && (ip_header->version_ihl & 0xF) > 5) {
		if (!handle_packet_timestamp(ip_header, router_ip, current_milliseconds_utc)) {
			return mbuf->port;
		}
	}

	uint8_t dst_device = mbuf->port == config.lan_main_device ? config.wan_device : config.lan_main_device;
