...
lb_allocate_balancer(uint32_t flow_capacity, uint32_t backend_capacity,
  ...
  if (map2_allocate(lb_ip_equality, lb_ip_hash, backend_capacity, &(balancer->ip_to_backend_id)) == 0) {
    goto err;
  }

  if (dchain2_allocate(backend_capacity, &(balancer->active_backends)) == 0) {
    goto err;
  }
  ...
  map2_set_layout(balancer->ip_to_backend_id, &uint32_field, 1, NULL, 0, "uint32_t");
  map2_set_entry_condition(balancer->ip_to_backend_id, lb_backend_id_condition);
...
lb_find_preferred_available_backend(uint64_t hash, struct Vector* cht,
  ...
  for (int i = 0; i < backend_capacity; ++i) {
    ...
    if (dchain2_is_index_allocated(active_backends, *candidate)) {
...
lb_get_backend(struct LoadBalancer* balancer, struct LoadBalancedFlow* flow, time_t now) {
  ...
  if (map_get_1(balancer->flow_to_flow_id, flow, &flow_index) == 0) {
    int backend_index;
    int found =
    ...
    if (0 == dchain2_is_index_allocated(balancer->active_backends, backend_index)) {
...
void lb_process_heartbit(struct LoadBalancer* balancer,
                         ...
                         time_t now) {
  int backend_index;
  if (map2_get_1(balancer->ip_to_backend_id, &flow->src_ip, &backend_index) == 0) {
    VIGOR_TAG(TRAFFIC_CLASS, NEW_BACKEND);
    if (0 != dchain2_allocate_new_index(balancer->active_backends,
                                       &backend_index, now)) {
      ...
      *ip = flow->src_ip;
      map2_put(balancer->ip_to_backend_id, ip, backend_index);
    ...
    //assert(dchain2_is_index_allocated(balancer->active_backends, backend_index));
    dchain2_rejuvenate_index(balancer->active_backends, backend_index, now);
  }
}
...
void lb_expire_backends(struct LoadBalancer* balancer, time_t now) {
  ...
  expire_items_single_map2(balancer->active_backends, balancer->backend_ips, balancer->ip_to_backend_id, last_time);
}

#ifdef KLEE_VERIFICATION

  __attribute__((noinline)) struct Map** lb_get_flow_to_flow_id(struct LoadBalancer* balancer) { return &(balancer->flow_to_flow_id); }
