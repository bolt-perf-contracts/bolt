...
pushd "$BUILDDIR"
  if [ ! -f dpdk/.version ] || [ "$(cat dpdk/.version)" != "$DPDK_RELEASE" ]; then
    ...
    pushd dpdk
      ...
      # The order is important. replay.dpdk.patch assumes the other patches are
      # applied.
...
sudo apt-get install -y --no-install-recommends \
                     ...
                     valac #\
                     #gtksourceview2.0-dev \
                     #liblablgtk2-ocaml-dev liblablgtksourceview2-ocaml-dev
                     # ^^ Necessary only for vfide
...
git clone --depth 1 --branch vigor-intrinsics https://github.com/rishabh246/klee.git "$BUILDDIR/klee"
pushd "$BUILDDIR/klee"
  mkdir build
  pushd build
